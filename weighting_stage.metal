...
// Stage 2: probabilistic weighting and best-lane selection
kernel void weighting_stage(
    ...
    constexpr uint TG_CAP = 16; // small to reduce frontend stress

    threadgroup atomic_uint tg_hit_count[1];
    threadgroup uint tg_results[TG_CAP*2]; // only store best_nonce + digest
